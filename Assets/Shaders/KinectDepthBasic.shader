Shader "Custom/KinectDepthBasic"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				float d = (col.w*sin(_Time.y*_Entropy) + col.z * 16 + col.y * 16 * 16 + col.x * 16 * 16 * 16) * _Displacement;
				...
				v.vertex.x = v.vertex.x * d / _Division *8;
				v.vertex.y = v.vertex.y * d / _Division *8;
				v.vertex.z = d ;
			...
			{
				// テクスチャの色をそのまま使う
				fixed4 colz = tex2D(_MainTex, i.uv);
				// 指定の色にする場合
				// fixed4 col = _Color; 
				// fixed4 col = i.col;
  ...
  //           float _Amount;
		...
		// 		//TextureFormat.RGBA4444の場合
		// 		float d = (col.w + col.z * 64 + col.y * 64 * 64 + col.x * 64 * 64 * 64) * _Displacement;
		// 		// float d = (col.r*_CutOutThresh *dcolor + col.g*_CutOutThresh*dcolor + col.b*_CutOutThresh*dcolor);
